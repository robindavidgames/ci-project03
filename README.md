# README for NaNoWriMo Progress Tracker
...
The target audience is NaNoWriMo particpants and it fulfils the need of tracking their progress and providing targets to work towards. 
...
### Google Sheets
A Google Sheets document hosts information relating to 10 fictional users. This information is imported whenever the program is run/restarted and can be added to and updated through user actions. The default user is User 1/Row 1 in the Google Sheet. Each day of writing is represented by a new column.
...
### Log Day
The log_day() function allows the user to post a new day's worth of writing progress. It automatically updates the next blank day and will not let the user input more than 30 days (as that is the maximum allowed in the National Novel Writing Month challenge). It calls a validation function to check the inputted value and then sends the validated data, along with the date, to a function to update the worksheet.

![Function allowing the user to add a new day.](./assets/readme/feature4-newday.png)
...
![Function allowing the user to update a previous day.](./assets/readme/feature5-prevday.png)
...
![Example of invalid data.](./assets/readme/feature6-validation.png)
...
In see_progress(), I needed to limit the number of pieces of data shown on the terminal (as there could be up to 30 pieces of data and the terminal is only 24 lines in height). Subsequently, a range set to len(data) would not be appropriate. When I tried setting the range to manual number (eg 20-30), it would result in a crash if there was less than 30 pieces of data (ie, if there were 28 days logged). To fix this error, I set the minimum at 20 and the maximum as len(data), so that the maximum boundary would be variable.
...
Although my code worked fine, GitPod was giving a warning that for loops with len and range in them are not ideal and suggested I use Enumerate. I used the tutorial at [RealPython](https://realpython.com/python-enumerate/) to learn about enumerating and turned this code:
